...
type Position = {
  id: string; type: "long" | "short"; size: number; stopLoss: number; entryPrice: number;
  protectionId: string | null; autoRenew: boolean; premium: number;
  status: "active" | "closed" | "triggered"; closedPnl: number | null; closedPayout: number | null;
};
...
const POSITION_MIN = 5000, POSITION_MAX = 50000, POSITION_STEP = 5000;
const PREMIUM_PER_K = 11, TENOR_DAYS = 5, INITIAL_BALANCE = 1_000_000;
const BALANCE_KEY = "foxify_pilot_balance", SETTLEMENT_KEY = "foxify_pilot_settlement", POSITIONS_KEY = "foxify_pilot_positions";
...
const fmtTime = (ms: number): string => { if (ms <= 0) return "Expired"; const h = Math.floor(ms / 3600000), m = Math.floor((ms % 3600000) / 60000); return h >= 24 ? `${Math.floor(h / 24)}d ${h % 24}h` : `${h}h ${m}m`; };
...
const api = async <T = unknown>(path: string, opts?: RequestInit): Promise<T> => {
  ...
  const json = await res.json(); if (!res.ok) throw new Error(json?.reason || json?.message || `HTTP ${res.status}`); return json as T;
};
...
const load = <T,>(key: string, fb: T): T => { try { const v = localStorage.getItem(key); return v ? JSON.parse(v) : fb; } catch { return fb; } };
const save = (key: string, v: unknown) => { try { localStorage.setItem(key, JSON.stringify(v)); } catch {} };

// ─── Collapsible section ─────────────────────────────────────────────

function Section({ title, badge, defaultOpen = true, children }: { title: string; badge?: string; defaultOpen?: boolean; children: React.ReactNode }) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div className="section">
      <button onClick={() => setOpen(o => !o)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 0 8px 0", border: "none", background: "none", cursor: "pointer", color: "var(--text)" }}>
        ...
        <span style={{ fontSize: 12, color: "var(--muted)", transition: "transform 0.2s ease", transform: open ? "rotate(180deg)" : "rotate(0deg)" }}>▼</span>
      </button>
      {open && children}
    </div>
  );
...
export function PilotWidget() {
  ...
  const floorPriceLocal = livePrice && stopLoss ? (positionType === "short" ? livePrice * (1 + drawdownPct / 100) : livePrice * (1 - drawdownPct / 100)) : null;
  ...
  const unrealizedPnl = activePositions.reduce((s, p) => { if (!livePrice) return s; return s + (p.type === "long" ? ((livePrice - p.entryPrice) / p.entryPrice) * p.size : ((p.entryPrice - livePrice) / p.entryPrice) * p.size); }, 0);
  const isPriceFresh = Date.now() - priceTs < 1500;

  // Poll BTC price
  useEffect(() => {
    let on = true;
    const poll = async () => { try { const d = await fetchRef(); if (on && d.status === "ok") { setLivePrice(Number(d.reference.price)); setPriceTs(Date.now()); setPriceError(null); } } catch (e: any) { if (on) setPriceError(e.message); } };
  ...
  // Poll monitors
  useEffect(() => {
    const pa = activePositions.filter(p => p.protectionId);
    if (!pa.length) return;
    let on = true;
    const poll = async () => {
      for (const pos of pa) {
  ...
  const handleOpenProtected = useCallback(async () => {
    ...
    try {
      const ep = livePrice, premium = (positionSize / 1000) * PREMIUM_PER_K;
      const q = await fetchQuote({ protectedNotional: positionSize, foxifyExposureNotional: positionSize, entryPrice: ep, tierName, drawdownFloorPct: drawdownPct / 100, protectionType: positionType });
      const r = await activateProtection({ quoteId: q.quote.quoteId, protectedNotional: positionSize, foxifyExposureNotional: positionSize, entryPrice: ep, tierName, drawdownFloorPct: drawdownPct / 100, autoRenew, protectionType: positionType });
      const pid = r.protectionId || r.protection?.id || null;
      setPositions(prev => [...prev, { id: `pos_${++posIdRef.current}_${Date.now()}`, type: positionType, size: positionSize, stopLoss: drawdownPct, entryPrice: ep, protectionId: pid, autoRenew, premium, status: "active", closedPnl: null, closedPayout: null }]);
  ...
  const handleOpenWithout = () => {
    if (!livePrice || !formComplete || !positionType || !stopLoss) return;
    setPositions(prev => [...prev, { id: `pos_${++posIdRef.current}_${Date.now()}`, type: positionType, size: positionSize, stopLoss: drawdownPct, entryPrice: livePrice, protectionId: null, autoRenew: false, premium: 0, status: "active", closedPnl: null, closedPayout: null }]);
    setPositionType(null); setStopLoss(null);
  };
      ...
      <div className="card" style={{ maxWidth: 500 }}>
        ...
        <div className="title">
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <img src={FOXIFY_LOGO} alt="" style={{ width: 24, height: 24, borderRadius: 6 }} onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
          ...
          {livePrice && (
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 600, fontVariantNumeric: "tabular-nums", transition: "color 0.3s", color: isPriceFresh ? "var(--text)" : "var(--muted)" }}>{fmtUsd(livePrice)}</span>
              {!priceError && <span style={{ width: 6, height: 6, borderRadius: "50%", display: "inline-block", background: isPriceFresh ? "var(--success)" : "var(--muted)", transition: "background 0.3s" }} />}
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", marginBottom: 14, borderRadius: 8, background: "var(--card-2)", border: "1px solid var(--border)" }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 700, fontVariantNumeric: "tabular-nums", color: (balance + unrealizedPnl) >= INITIAL_BALANCE ? "var(--success)" : "var(--text)" }}>{fmtUsd(balance + unrealizedPnl)}</span>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          {(["long", "short"] as const).map(side => (
            <button key={side} onClick={() => setPositionType(side)} style={{
              flex: 1, padding: "10px 0", borderRadius: 10, cursor: "pointer", fontSize: 13, fontWeight: 600, textTransform: "capitalize",
              border: positionType === side ? side === "long" ? "1.5px solid var(--success)" : "1.5px solid var(--danger)" : "1px solid var(--border)",
              background: positionType === side ? side === "long" ? "rgba(54,211,141,0.15)" : "rgba(255,107,107,0.15)" : "var(--card-2)",
              color: positionType === side ? side === "long" ? "var(--success)" : "var(--danger)" : "var(--muted)", transition: "all 0.15s ease",
            }}>{side}</button>
          ))}
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 0, borderRadius: 8, overflow: "hidden", border: "1px solid var(--border)" }}>
            <button onClick={() => setPositionSize(s => Math.max(POSITION_MIN, s - POSITION_STEP))} disabled={positionSize <= POSITION_MIN} style={{ width: 36, height: 36, border: "none", cursor: "pointer", background: "var(--card-2)", color: "var(--text)", fontSize: 18, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", opacity: positionSize <= POSITION_MIN ? 0.3 : 1 }}>−</button>
            ...
            <button onClick={() => setPositionSize(s => Math.min(POSITION_MAX, s + POSITION_STEP))} disabled={positionSize >= POSITION_MAX} style={{ width: 36, height: 36, border: "none", cursor: "pointer", background: "var(--card-2)", color: "var(--text)", fontSize: 18, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "center", opacity: positionSize >= POSITION_MAX ? 0.3 : 1 }}>+</button>
          </div>
        </div>
        ...
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12, marginTop: 4 }}>
          ...
          <div style={{ background: "rgba(54,211,141,0.06)", border: "1px solid rgba(54,211,141,0.18)", borderRadius: 12, padding: 14, marginBottom: 12, opacity: formComplete ? 1 : 0.5 }}>
            ...
            {formComplete && <>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--muted)" }}><span>Premium</span><span style={{ fontWeight: 600, color: "var(--text)" }}>{fmtUsd(computedPremium)} for {TENOR_DAYS} days</span></div>
              {floorPriceLocal && <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--muted)", marginTop: 4 }}><span>{positionType === "long" ? "Floor Price" : "Ceiling Price"}</span><span style={{ fontWeight: 500 }}>{fmtUsd(floorPriceLocal)}</span></div>}
            </>}
          </div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={handleOpenProtected} disabled={!formComplete || activating || !livePrice} style={{ flex: 2, padding: "12px 0", borderRadius: 10, border: "none", fontSize: 14, fontWeight: 600, cursor: "pointer", background: "linear-gradient(135deg, var(--accent), var(--accent-2))", color: "#fff", opacity: (!formComplete || activating || !livePrice) ? 0.5 : 1, transition: "opacity 0.15s ease" }}>
              {activating ? "Opening..." : "Open + Protect"}
            </button>
            <button onClick={handleOpenWithout} disabled={!formComplete || !livePrice} style={{ flex: 1, padding: "12px 0", borderRadius: 10, border: "1px solid var(--border)", background: "var(--card-2)", fontSize: 12, color: "var(--muted)", cursor: "pointer", opacity: (!formComplete || !livePrice) ? 0.5 : 1 }}>
              Open Without
            </button>
          </div>
        </div>
        ...
        {activePositions.length > 0 && (
          <Section title="Active Positions" badge={`${activePositions.length}`}>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {activePositions.map(pos => {
                ...
                const floor = pos.type === "long" ? pos.entryPrice * (1 - pos.stopLoss / 100) : pos.entryPrice * (1 + pos.stopLoss / 100);
                return (
                  <div key={pos.id} style={{ padding: 12, borderRadius: 10, border: "1px solid var(--border)", background: "var(--card-2)" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--muted)", marginBottom: 4 }}>
                      ...
                      {pnl !== null && <span style={{ fontWeight: 600, color: pnl >= 0 ? "var(--success)" : "var(--danger)", fontVariantNumeric: "tabular-nums" }}>{fmtUsd(pnl)} ({fmtPct(pnlPct!)})</span>}
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--muted)", marginBottom: 6 }}>
                      <span>{pos.type === "long" ? "Floor" : "Ceiling"}: {fmtUsd(floor)}</span>
                      {pos.protectionId && mon?.timeRemaining && <span>{fmtTime(mon.timeRemaining.ms)} left</span>}
                      {pos.premium > 0 && <span>Premium: {fmtUsd(pos.premium)}</span>}
                    </div>
                    <button onClick={() => handleClose(pos.id)} style={{ width: "100%", padding: "6px 0", borderRadius: 6, border: "1px solid var(--border)", background: "transparent", fontSize: 11, color: "var(--muted)", cursor: "pointer" }}>Close Position</button>
                  </div>
                );
              })}
            </div>
        ...
        {closedPositions.length > 0 && (
          <Section title="Recently Closed" badge={`${closedPositions.length}`} defaultOpen={false}>
              ...
              {closedPositions.slice(-5).map(pos => (
                <div key={pos.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", borderRadius: 8, background: "var(--card-2)", border: "1px solid var(--border)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {pos.status === "triggered" && <span style={{ fontSize: 10, fontWeight: 600, padding: "1px 5px", borderRadius: 999, background: "rgba(54,211,141,0.12)", color: "var(--success)" }}>Payout: {fmtUsd(pos.closedPayout || 0)}</span>}
                    {pos.status === "closed" && pos.closedPnl !== null && <span style={{ fontSize: 11, fontWeight: 600, color: pos.closedPnl >= 0 ? "var(--success)" : "var(--danger)" }}>P&L: {fmtUsd(pos.closedPnl)}</span>}
                  </div>
        ...
        {(settlement.totalPremiums > 0 || settlement.totalPayouts > 0) && (
          <Section title="Pilot Settlement">
            <div style={{ display: "flex", flexDirection: "column", gap: 4, fontSize: 12, color: "var(--muted)" }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}><span>Premiums owed</span><span style={{ fontWeight: 500, color: "var(--text)" }}>{fmtUsd(settlement.totalPremiums)}</span></div>
              <div style={{ display: "flex", justifyContent: "space-between" }}><span>Payouts earned</span><span style={{ fontWeight: 500, color: settlement.totalPayouts > 0 ? "var(--success)" : "var(--text)" }}>{fmtUsd(settlement.totalPayouts)}</span></div>
              <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid var(--border)", paddingTop: 4, marginTop: 2 }}>
                ...
                <span style={{ fontWeight: 700, color: settlement.totalPayouts - settlement.totalPremiums >= 0 ? "var(--success)" : "var(--danger)" }}>{fmtUsd(settlement.totalPayouts - settlement.totalPremiums)}</span>
              </div>
            </div>
          </Section>
        )}
