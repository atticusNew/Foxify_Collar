...
import { solveAndPriceCreditCollar, type CreditCollarParams, type PerpSide } from "./creditCollarPricer";
...
export type CostModel = {
  ...
  optionRelHalfSpreadPct: number;       // per-leg half-spread as fraction of mid premium
  ...
  perpHalfSpreadBps: number;            // perp crossing, bps of notional
  ...
  stressJumpPct: number;                // correlated jump for the reserve (e.g. 0.12 = −12% day)
};
...
export const compareHedgeArchitectures = (book: BookSpec, cost: CostModel): ArchComparison => {
  ...
  const residualDeltaNotional = Math.abs(longNotional - shortNotional); // hedged with perps in internalize mode
  const maxSideShare = grossNotional > 0 ? Math.max(longNotional, shortNotional) / grossNotional : 0.5;
  ...
  const perpHedgeCostPerDay = (residualDeltaNotional * cost.perpHalfSpreadBps) / 1e4 * cost.dailyRehedgeTurnover;
  // Reserve covers the gross netted book on the imbalanced jump with the delta hedge FAILING over the gap.
  const intReserve = grossNotional * maxSideShare * stressExcess * cost.reserveMultiple;
  ...
  //    (small) reserve on the warehoused, un-back-to-backed fraction. ──
  const b2bWarehoused = grossNotional * cost.backToBackWarehouseFraction;
  const b2bReserve = b2bWarehoused * maxSideShare * stressExcess * cost.reserveMultiple;
