...
test("Parity probe captures both venues with all 8 protection probes", { skip: SKIP }, async () => {
...
test("At least one row has both Bullish and Deribit data", { skip: SKIP }, async () => {
  const snapshot = await runParityProbe();
...
test("2% tier always has both venues (essential for multi-venue routing)", { skip: SKIP }, async () => {
  const snapshot = await runParityProbe();
...
test("Snapshot includes operator notes for unavailable strikes", { skip: SKIP }, async () => {
  const snapshot = await runParityProbe();
