 ...
 *
 * Focus: the amount-snapping fix (Deribit BTC options trade in 0.01 BTC steps, min 0.01).
 * Without snapping, a full-precision size like 0.35211 BTC (= notional/spot) is rejected by
 * Deribit → live order never fills. Also covers the USDC↔BTC price conversion + spot guard.
 */
...
test("buyLeg: snaps full-precision 0.35211 → 0.35 BTC (nearest 0.01 step) and converts USDC→BTC price", async () => {
  const cap: Captured = {};
  ...
  assert.equal(cap.order?.amount, 0.35, "amount snapped to a valid 0.01 multiple");
...
test("buyLeg: a valid 0.01-multiple (0.07) passes through unchanged", async () => {
  const cap: Captured = {};
  const r = await adapter(cap, 73_000).buyLeg({ instrument: "i", contractsBtc: 0.07, maxAcceptableAskUsdcPerBtc: 1_000, clientOrderId: "x" });
  assert.equal(r.ok, true);
  assert.equal(cap.order?.amount, 0.07);
});

test("buyLeg: amount below venue min (0.003 → 0) is REJECTED before any order is placed", async () => {
  const cap: Captured = {};
  const r = await adapter(cap, 73_000).buyLeg({ instrument: "i", contractsBtc: 0.003, maxAcceptableAskUsdcPerBtc: 1_000, clientOrderId: "x" });
  assert.equal(r.ok, false);
  if (!r.ok) assert.match(r.detail ?? "", /below venue min/);
  assert.equal(cap.order, undefined, "no order placed for sub-min size");
});

test("sellLeg: snaps amount to 0.01 step (0.35211 → 0.35) so close matches the held size", async () => {
  const cap: Captured = {};
  ...
  assert.equal(r.ok, true);
  assert.equal(cap.order?.amount, 0.35);
