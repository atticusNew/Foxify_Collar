...
import { loadPrincipalPairs } from "../src/singleSide/twoSided/creditCollar/execution/principalPairRunner";
...
const server = createServer((req, res) => {
  const out = handleDashboardRequest(
    ...
    { loadRecords: () => loadScorecards(), liveStatus: () => status, settlementAggregate: () => loadSettlementAggregate(), foxifyView: (feeUsdc?: number) => loadFoxifyView(undefined, { perpFeeUsdc: feeUsdc ?? foxifyPerpFeeUsdc, venues: foxifyVenues }), regimeStats: (feeUsdc?: number) => loadRegimeStats(undefined, { perpFeeUsdc: feeUsdc ?? foxifyPerpFeeUsdc, gate: cfg.regimeGate, liveGaugePct: computeLiveRegimeSignal(loadPriceHistory(), Date.now(), { lookbackMs: cfg.regimeGate?.liveLookbackMs, minSamples: cfg.regimeGate?.liveMinSamples })?.gaugePct ?? null, prevRegime: loadGateState()?.regime ?? null }), positions: () => ({ open: loadOpenPositions(), settled: loadSettlements() }), lifecycleReport: () => latestLifecycle, token, principalPairs: () => loadPrincipalPairs(process.env.PRINCIPAL_PAIRS_PATH), oneSheetContact: process.env.SHADOW_CONTACT || undefined, showDirectionalTracker: String(process.env.SHADOW_SHOW_DIRECTIONAL_TRACKER ?? "").toLowerCase() === "true", productBookSinceMs: Number.isFinite(Date.parse(String(process.env.SHADOW_PRODUCT_BOOK_SINCE ?? ""))) ? Date.parse(String(process.env.SHADOW_PRODUCT_BOOK_SINCE)) : undefined, aggregateConfig: { exposureBandPct: haltBand, targetServiceFeeBps: cfg.serviceFeeBps, capital: capitalConfig } }
  );
