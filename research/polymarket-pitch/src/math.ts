/** Self-contained BS + spread math. No pilot dependencies. */
...
export function putSpreadCost(S: number, K_long: number, K_short: number, T: number, r: number, ivLong: number, ivShort: number): number {
  if (K_long <= K_short) return 0;
  return Math.max(0, bsPut(S, K_long, T, r, ivLong) - bsPut(S, K_short, T, r, ivShort));
}
export function callSpreadCost(S: number, K_long: number, K_short: number, T: number, r: number, ivLong: number, ivShort: number): number {
  if (K_short <= K_long) return 0;
  return Math.max(0, bsCall(S, K_long, T, r, ivLong) - bsCall(S, K_short, T, r, ivShort));
}
export function putSpreadPayout(K_long: number, K_short: number, S_entry: number, S_expiry: number, notional: number): number {
  if (K_long <= K_short || S_expiry >= K_long) return 0;
  const gain = Math.min(K_long - S_expiry, K_long - K_short);
  return (gain / S_entry) * notional;
}
export function callSpreadPayout(K_long: number, K_short: number, S_entry: number, S_expiry: number, notional: number): number {
