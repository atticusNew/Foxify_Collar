...
- Position sizing: typical retail HL positions cluster ~$500–$10k. Positions above the per-wallet cap are wrapped **partially** — wrap min(position notional, remaining per-wallet cap), rounded DOWN to whole OKX lots (0.01 BTC ≈ $630; rounding up would over-hedge and create naked exposure). UI must state partial coverage plainly: "Protected: $1,890 of your $31,000 position — coverage limits rise as capacity grows."
...
4. **Caps are formulas, not fixed numbers** — derived in one config module so raising capital raises every cap by changing one input:
   - **Book cap** = usable margin ÷ measured per-wrap margin rate, where usable margin = 60% of sub-account capital (the auto-pause line; renewals and breach unwinds always keep 40% headroom). Launch estimate at $10k capital and ~12% margin/notional: **~$50k book**. MUST be recalibrated when Phase 0 measures real PM margin per wrap.
   - **Per-WALLET cap** (not per-wrap — one whale must not sidestep it with multiple positions) = book cap ÷ target concurrent wallets (launch target 25) → **~$2k ≈ 3 OKX lots** at launch; floor of 1 lot so small positions always fit.
   - **Per-strike concentration cap: ≤30% of book notional short any single strike/expiry.** Knockout unwinds execute on the order book (see decision 7), so simultaneous same-strike knockouts are the real tail risk, not book size.
   - **50-wallet waitlist** (founding cohort) beyond which new wallets queue.
...
7. **Execution asymmetry — RFQ in, screen out (per OKX BD).** Portfolio margin affects capital efficiency only, never fills. RFQ/block trades have minimum sizes single retail wraps won't reach, so RFQ is a BOOK-LEVEL tool: batch/net the book's aggregate hedge needs (longs offset shorts, same-strike wraps combine) and execute net deltas as blocks above the minimum (confirm exact OKX options block minimums with the BD rep). Knockout unwinds are urgent and go through the order book ("the screen") — this is why the per-strike concentration cap exists and why the renewal loop must STAGGER renewals rather than batching them onto one tick (staggered renewals land on different strikes as spot moves, naturally de-concentrating the book).
...
- **Staggered renewals** (decision 7): renewals must NOT batch onto one tick — spread them across the cycle (e.g., each wrap renews at its own anchor time) so the book naturally lands on different strikes as spot moves.
...
- **Caps as a config module** (decision 4): one module derives book cap, per-wallet cap, and per-strike concentration cap from two inputs (sub-account capital, measured per-wrap margin rate) plus targets (concurrent wallets, headroom %). Wire into `DemoGuardsConfig`/`assessDemoWrap`; add the 50-wallet waitlist and margin-utilization auto-pause. All inputs env-configurable. A single kill switch env/endpoint pauses all new wraps and renewals while leaving conclusions/payouts running.
...
- OKX PM/RFQ: verify both legs margin as a netted spread under PM and **measure real margin per wrap** (this recalibrates every cap in decision 4). RFQ per decision 7: book-level batching of netted hedge needs into blocks, entries only; confirm OKX options block minimums with the BD rep. Knockout unwinds stay on the order book.
