...
0. **`EMPIRICAL_COOLDOWN_FINDINGS.md`** — **READ THIS FIRST. Supersedes earlier cooldown analysis.** Cooldown plumbed into `historical_replay.py` and run across the full 6.4-year BTC + 5-year DVOL tape under five threshold configurations. **Empirical finding: cooldown clips 0.0–0.6 % of triggers under spec as written (vs 50 % assumed in earlier docs); tightening thresholds raises clip to 5 % at most but COSTS Atticus $0–$17 M/yr by holding stale strangles.** Conclusion: **the absolute best price without cooldown is `$490 / $720 / $1,025 / $1,455` per pair per day** (~5 % uniform Atticus margin); cooldown stays as Atticus-side tail-risk control with current loose spec thresholds, NOT as a commercial pricing lever. Foxify pays ~$47 M/yr at 1,000 pairs (1.49 bps), about 12 % more than the empirically-invalid cooldown'd ladder, but with zero service interruptions, zero payout clipping, and zero month-1 reconciliation risk.
0. **`COOLDOWN_FOXIFY_BREAKDOWN.md`** — **Customer-facing cooldown explainer.** Plain-English: when cooldown fires (T1–T4), what Foxify experiences during a window, per-regime firing frequency, payout-clip estimates, worked stress-day timeline. **Note:** the per-regime payout-clip estimates in §4 of this doc were the analytical priors; the empirical truth is in `EMPIRICAL_COOLDOWN_FINDINGS.md` §3.
0. **`PRICING_LADDER_STRESS_TEST.md`** — **Quant validation of the cooldown'd ladder.** Documented one open analytical risk: cooldown's design-target clip needs empirical calibration. That calibration has now been done; see `EMPIRICAL_COOLDOWN_FINDINGS.md` for the answer.
