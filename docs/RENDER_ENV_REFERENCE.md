...
| `SS_TWO_SIDED_LIVE_ENABLED` | bool | `false` | Master switch for REAL-MONEY execution. **ENFORCED** (2026-06-01): when `FOXIFY_V2_LIVE_EXECUTION=true`, any non-shadow activation is blocked `503 live_flag_disabled` unless this is `true`. Stays `false` during shadow testing. |
| `FOXIFY_V2_LIVE_EXECUTION` | bool | `false` | **THE execution gate** — when `true`, the server wires the LIVE executor (real venue orders) AND activates the live safety rails (below). Both this AND `SS_TWO_SIDED_LIVE_ENABLED` must be `true` for real money to execute (defense in depth). Read at boot → requires redeploy to change. |
| `SS_TWO_SIDED_BOOT_HALT` | bool | `true` | System auto-halts on every deploy boot. Operator clears with `POST /admin/foxify/v2/resume`. Belt-and-suspenders. OK to set `false` while purely in shadow; set back to `true` before the live flip. |
| `SS_TWO_SIDED_MAX_PAIRS_PER_DAY` | int | `2` | **ENFORCED** hard cap on real-money (non-shadow) activations per UTC day — over-cap returns `503 daily_cap_reached`. Increase incrementally as confidence grows. |
| `SS_TWO_SIDED_CELL_ALLOWLIST` | csv | `pair_50k_2pct` | **ENFORCED** LIVE cell gate (CSV, no quotes/brackets, e.g. `pair_a,pair_b`). When live execution is on, only listed cells may fire a real pair (else `503 cell_not_in_allowlist`). Separate from the per-regime DB allowlist (which gates shadow + regime eligibility, overridable via `/admin/foxify/v2/cell-allowlist`). |
...
| `SS_TWO_SIDED_CALM_LOSS_LEADER` | bool | `false` | **Budgeted calm volume mode.** When `true`, calm activation is permitted for allowlisted cells **only if** the premium (the position's bounded max loss) ≤ `SS_TWO_SIDED_CALM_MAX_LOSS_USDC`. Lets the CEO buy perp volume in calm at a known, capped per-pair cost; loss shrinks as DVOL rises (see `GET /admin/foxify/v2/breakeven-ladder`). Independent of `SS_TWO_SIDED_ALLOW_CALM`. `should_activate` surfaces a `calm_loss_leader` block (with `eligible_cells`) so an external bot can opt in. **The shadow auto-loop also AUTO-FIRES these cells in calm when this flag is on** (tries the 2d primary, falls through to the 1d if over budget), subject to the calm-shadow flag + daily cap + rate-limit window — so volume accrues autonomously without an external bot. (Calm LIVE still also gated by `SS_TWO_SIDED_LIVE_ENABLED`; the auto-loop fires SHADOW.) **Wired loss-leader cells (calm allowlist):** `pair_25k_5otm_strangle_2d` (PRIMARY — premium ~$52, breakeven DVOL ~45, +$20 by DVOL 50) and `pair_25k_5otm_strangle_1d` (cheapest deep-calm — premium ~$18, breakeven DVOL ~50). Hedge legs route to Deribit (Bullish has no resting book at the 5% OTM wings). |
| `SS_TWO_SIDED_CALM_MAX_LOSS_USDC` | int | `55` | Per-pair max premium (max loss) for the calm loss-leader. Activations above this are blocked `calm_loss_exceeds_budget`. **Default `55`** admits BOTH wired cells at current vol: the 2d primary (~$52 premium, breakeven DVOL ~45) and the cheaper 1d (~$18). Use `45` for a 1d-only deep-calm posture, or raise to widen. |
...
| `SS_MC_BOOTSTRAP_ALL_REGIMES` | `false` | Fat-tail MC: when `true`, ALL regimes simulate paths via a real-5min-bar bootstrap (preserves BTC fat tails/autocorrelation) scaled to the regime σ, instead of GBM for non-calm. Default `false` keeps GBM (validated sweep numbers unchanged). **Flip deliberately, then re-run the sweep to re-validate** — it will shift the moderate/elevated/stress net estimates. Falls back to GBM if bars unavailable. |
