...
| `SS_TWO_SIDED_CALM_LOSS_LEADER` | bool | `false` | **Budgeted calm volume mode.** When `true`, calm activation is permitted for allowlisted cells **only if** the premium (the position's bounded max loss) ≤ `SS_TWO_SIDED_CALM_MAX_LOSS_USDC`. Lets the CEO buy perp volume in calm at a known, capped per-pair cost; loss shrinks as DVOL rises (see `GET /admin/foxify/v2/breakeven-ladder`). Independent of `SS_TWO_SIDED_ALLOW_CALM`. `should_activate` surfaces a `calm_loss_leader` block (with `eligible_cells`) so the bot can opt in. (Calm LIVE still also gated by `SS_TWO_SIDED_LIVE_ENABLED`.) **Wired loss-leader cells (calm allowlist):** `pair_25k_5otm_strangle_2d` (PRIMARY — cost ~$40, breakeven DVOL ~42.6, ramps +$145 by DVOL 60) and `pair_25k_5otm_strangle_1d` (cheapest deep-calm — cost ~$12.50, breakeven DVOL ~50). |
| `SS_TWO_SIDED_CALM_MAX_LOSS_USDC` | int | `25` | Per-pair max premium (max loss) for the calm loss-leader. Activations above this are blocked `calm_loss_exceeds_budget`. **Recommended `45`** to admit the 2d primary (~$40) with headroom + the 1d; use `15` for a 1d-only deep-calm posture. |
